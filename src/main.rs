...
fn init_stars(count: usize) -> Vec<Star> {
    ...
    (0..count)
        .map(|_| Star {
            // Start each star somewhere inside a normalized 3D box.
...
async fn main() {
    ...
    loop {
        ...
        draw_circle(center_x, center_y, 20.0, RED);
        draw_text("DEBUG", 20.0, 80.0, 30.0, YELLOW);
        ...
        for star in stars.iter_mut() {
            ...
            if star.z < 0.2 {
                star.z = STAR_DEPTH;
                // Respawn at a new horizontal position near the back of the field.
